...
pub trait PostsetConnectable {
    ...
    /// Returns an immutable reference the set of nodes with outgoing connections to this node.
    fn get_postset(&self) -> &BTreeSet<Self::RefType>;

    /// Returns a mutable reference the set of nodes with outgoing connections to this node.
